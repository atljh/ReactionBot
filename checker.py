...
async def check_account(session_file: Path, json_data: dict) -> tuple:
...
async def main():
    ...
    if args.phone:
        ...
        phone, status, username = await check_account(session_file, json_data)
    ...
    async def check_with_semaphore(session_file, json_file, json_data):
        async with semaphore:
            return await check_account(session_file, json_data)
    ...
    for phone, status, username in sorted(results):
        if status == "OK":
